...
双击产物 `dist/RimeQ-0.1.5-preview.pkg` 安装，包含 Intel 与 Apple Silicon 程序。安装后在后台自动启用，完成后即可从菜单选择 **Rime Q**。只有启用未通过时才显示提示，提供稍后处理或注销选项，并安排下次登录重试。

输入法菜单提供设置、使用说明、检查更新和卸载。更新检查由用户主动触发，卸载默认保留个人词库。

设置页和“关于 Rime Q”显示当前版本及构建号。安装器自动区分首次安装、升级和同版修复，并阻止降级覆盖。候选栏按文字和注释收紧宽度，使用原生半透明材质。
