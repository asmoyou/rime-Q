...
**免费开源，官方版本无需购买。** [项目主页](https://github.com/asmoyou/rime-Q) · [官方发布与免费下载](https://github.com/asmoyou/rime-Q/releases)
...
双击产物 `dist/RimeQ-0.2.4-preview.pkg` 安装，包含 Intel 与 Apple Silicon 程序。安装后在后台自动启用，完成后即可从菜单选择 **Rime Q**。升级与同版修复会刷新自己的输入源启用状态。只有启用未通过时才显示提示，并安排下次登录重试。
...
设置页和“关于 Rime Q”显示当前版本、构建号与免费开源项目入口。安装器自动区分首次安装、升级和同版修复，并阻止降级覆盖。候选栏按文字和注释收紧宽度，采用圆角背景。“简洁皮肤”提供六款配色；“高级皮肤”以独立大卡片预览动态效果。“敲敲猫”趴在候选栏上边缘外侧，随按键敲击，不占候选内容空间。设置窗口可调整大小，内容按可用宽度换行与滚动。

内置雾凇拼音的日期时间、农历、计算器、数字金额、Unicode 和 UUID 工具，以及错音提示、英文自动大写、部件拆字、以词定字等辅助功能。中文模式下可输入 `rq`、`sj`、`nl`、`cC1+2`、`R123.45` 等触发，详见随包使用说明。
