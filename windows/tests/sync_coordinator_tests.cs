...
namespace RimeQ {
    ...
    internal static class SyncCoordinatorTests {
        ...
        static async Task Run(){
            ...
            var schedule=new SyncExportSchedule();
            Require(!schedule.Due(900000),"Clean dictionary scheduled an export");
            schedule.Observe("1",1000);Require(!schedule.Due(60999)&&schedule.Due(61000),"Quiet learning window boundary failed");
            schedule.Reset();schedule.Observe("1",1000);
            for(int i=1;i<=29;i++)schedule.Observe((i+1).ToString(),1000+i*10000);
            Require(!schedule.Due(300999)&&schedule.Due(301000),"Continuous learning did not respect five-minute bound");
            schedule.Reset();Require(!schedule.Due(900000),"Completed export left a dirty schedule");
            ...
            typeof(DeviceSync).GetField("nextCheck",flags).SetValue(null,0L);
            typeof(DeviceSync).GetField("nextExport",flags).SetValue(null,0L);
            int exportCount=exports;await DeviceSync.Tick();
            Require(exports==exportCount,"Own completed capture caused another full export");
            seed[0].weight+=1;Write(seed);
            typeof(DeviceSync).GetField("nextCheck",flags).SetValue(null,0L);
            await DeviceSync.Tick();Require(exports==exportCount,"Local learning skipped the quiet coalescing window");
            await DeviceSync.Tick(true);Require(exports>exportCount,"Manual export failed to bypass coalescing");
