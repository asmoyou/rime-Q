...
namespace RimeQ {
    internal static class Program {
        ...
        internal static void About() {
            ...
            var title=new TextBlock { Text="Rime Q "+Paths.Version,FontSize=22,FontWeight=FontWeights.SemiBold };
            var detail=new TextBlock { Text="Windows x64 · 构建 "+Paths.Build+"\n\n简洁、流畅、离线的中文输入法。\n基于 librime 构建，个人输入与学习保存在本机。",TextWrapping=TextWrapping.Wrap };
            var github=new Button { Content="GitHub 项目",MinWidth=100,Margin=new Thickness(0,0,8,0) };github.Click+=(s,e)=>Paths.Open(Paths.Project);
            var license=new Button { Content="许可条款",MinWidth=90,Margin=new Thickness(0,0,8,0) };license.Click+=(s,e)=>Paths.Open(Path.Combine(Paths.App,"licenses"));
            var close=new Button { Content="关闭",MinWidth=80,IsDefault=true };close.Click+=(s,e)=>window.Close();
            var buttons=new StackPanel { Orientation=Orientation.Horizontal,HorizontalAlignment=HorizontalAlignment.Right };buttons.Children.Add(github);buttons.Children.Add(license);buttons.Children.Add(close);
            var panel=new StackPanel { Margin=new Thickness(26) };panel.Children.Add(title);detail.Margin=new Thickness(0,12,0,20);panel.Children.Add(detail);panel.Children.Add(buttons);window.Content=panel;window.ShowDialog();
        }
        ...
        static int Main(string[] arguments) {
            try {
                ...
                if (action == "--about") { About(); return 0; }
                ...
                using (var mutex = new Mutex(true, prefix + "Owner", out created)) {
                    if (!created) {
                        if (action != "--background") {
                            Paths.Set("RequestedPage", action == "--updates" ? "3" : "-1");
                    ...
                    using (var quit = new EventWaitHandle(false, EventResetMode.AutoReset, prefix + "Quit", out created, security)) {
                        ...
                        menu.Items.Add("关于 Rime Q…", null, (s,e) => About());
                        var updateItem=menu.Items.Add("检查更新…", null, (s,e) => Show(3));
                        ...
                        app.Startup += async (s,e) => {
                            await model.Restore();
                            if(resources.NeedsRefresh) try {
                                await resources.Apply(resources.CopyConfiguration(),message=>Paths.Set("DictionaryStatus",message));
                                Paths.Set("DictionaryStatus","内置资源已更新，词库配置已重新应用。");
