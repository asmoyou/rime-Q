...
Windows 客户端源码位于 `windows/`，使用原生 TSF 接入，独立进程运行 librime，设置使用 WPF。版本 **0.4.1**；安装包名为 `RimeQ-0.4.1-windows-x64.exe`，下载入口为 [GitHub Releases](https://github.com/asmoyou/rime-Q/releases)。系统级安装与外部宿主验收的实际状态见 [验证记录](VALIDATION.md)，不要将隔离测试等同于所有应用可用。
...
开发构建可加 `--no-package`；用 `--build N` 指定递增的 Windows 文件构建号，范围 1–65535。默认构建号见脚本。产物为 `dist/RimeQ-0.4.1-windows-x64.exe` 和对应 SHA256SUMS；`build-windows/stage` 是打包目录，不应直接注册为系统安装。
...
`windows/tests/candidate_tests.cpp` 检查候选测量、长注释、鼠标索引、装饰尺寸/透传/隐藏和边缘避让，并渲染候选表面。输出位于忽略目录 `artifacts/windows-ui`。设置测试使用隔离偏好目录，覆盖五页、深浅主题、三种尺寸、个人词库状态机、第三方词库真实编译与启停候选，以及皮肤/字号控件和动画停止；当前桌面会话若无法产生非透明 WPF 位图，会明确报告而不把空图计作目视通过。
