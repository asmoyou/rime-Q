...
- 保留原来的单个 Rime Q 输入源和 Q 图标，另用“中 / A”状态按钮显示输入模式。单独按下并松开 Shift，点按状态按钮，或在输入法菜单切换“英文输入”。可按住 Command 将状态按钮拖到系统输入法图标左侧，位置由 macOS 保存；切换到其他输入法后隐藏状态按钮。可在系统输入法菜单选择“隐藏输入法名称”，以“中 Q / A Q”减少菜单栏占用。
