...
构建得到 `dist/RimeQ-0.2.2-preview.pkg`，双击即可安装应用与其内置的引擎、词库和模型。系统安装位置固定为 `/Library/Input Methods/RimeQ.app`，禁用 Installer 的应用重定位。安装后由 LaunchServices 启动当前登录用户的应用来注册输入法，再通过独立进程核验启用状态；不从包脚本直接执行注册入口。更新时会正常退出旧版本进程。
...
- 单独按下并松开 Shift 切换中英文。
- 输入法菜单的“设置”采用侧栏导航，提供“输入与外观”“皮肤”“个人词库”“词库与模型”。整句优化是万象语法模型开关，两种状态共用雾凇词库和个人学习记录；也可调整候选字号并即时预览。
- “皮肤”提供随系统、纸白、雾蓝、青玉、浅樱、暮色六款候选栏皮肤；选择即保存，下次输入生效。窗口缩小时皮肤卡片变为两列，设置内容可以滚动。
