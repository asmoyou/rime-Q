...
- 使用已具备的自动化权限，通过系统正常 Command 拖动将状态项移到原输入源左侧，AX 边界间距为 6 点。但切到美国键盘再切回后位置丢失，未将单次拖动写成位置保持通过。
...
- 位置修正版 `0.4.1 (1789361514)` 已通过双架构编译、控制器回归及包检查，SHA-256 为 `fcfc58147c2e032b9a615c94963166f1fd33ae4f088fabe78573ce4d457c01ec`。输入源注册元数据与前一恢复包完全相同。系统安装认证已发起，现用单输入源包已归档；最终安装后的相邻位置及隐藏/恢复验收仍待完成。0.4.1 保留草稿，最终 CI 与交付结果完成后补记。
