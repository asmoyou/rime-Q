...
## 2026-09-14 纠错包安装后待排查：会话内菜单显示

- 后续检查确认 `/Library/Input Methods/RimeQ.app` 已更新为纠错构建 `1789368955`；安装状态记录为 `ready`、服务应答通过、登录重试关闭。用户报告最近几次更新后系统输入法菜单不显示，需要注销再登录才能恢复。该反馈不能由安装状态或服务应答通过来否定，当前仍待核对本次登录会话和菜单 UI。
- 安装命令返回过 `shell-init: error retrieving current directory: getcwd: cannot access parent directories: Operation not permitted`，但实际应用已更新；该错误的来源与菜单问题是否相关尚未确定。后续以实际安装目录、进程、输入源及菜单证据判断，不重复安装或提前认定根因。
- 上一节“未安装”是构建验证结束时的状态。本节记录后续安装事实；真实宿主纠错与同会话安装后菜单恢复仍未验收。
