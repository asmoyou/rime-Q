...
def run(installed=None):
    output = ROOT / 'build-windows'
    stage = Path(installed).resolve() if installed else output / 'stage'
    ...
    for arch in ['x64', 'x86']:
        with tempfile.TemporaryDirectory(prefix='rimeq-tsf-') as temporary:
            ...
            try:
                ...
                print('Passed ' + ('installed ' if installed else 'production ') + arch + ' TIP with isolated x64 librime', flush=True)
            finally:
